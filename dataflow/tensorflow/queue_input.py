...
class QueueInputMulti(threading.Thread):
    def __init__(self, ds, placeholders, repeat_infinite=False, queue_size=50):
        ...
        self.queue = [tf.FIFOQueue(
            queue_size,
            [ph.dtype for ph in phs], shapes=[ph.get_shape() for ph in phs]
        ) for phs in placeholders]

        self.op = [self.queue[idx].enqueue(phs) for idx, phs in enumerate(placeholders)]
        self.close_op = [q.close(cancel_pending_enqueues=True) for q in self.queue]
            ...
            try:
                ...
                while True:
                    for idx, ph in enumerate(self.placeholders):
                        ...
                        dp = next(self._itr)
                        feed = dict(zip(ph, dp))
                        self.op[idx].run(feed_dict=feed)
            except (tf.errors.CancelledError, tf.errors.OutOfRangeError, df.DataFlowTerminated):
                pass
            except Exception as e:
                if isinstance(e, RuntimeError) and 'closed Session' in str(e):
    ...
    def dequeue(self, idx):
        return self.queue[idx].dequeue()

    def tensors(self, idx):
        return self.dequeue(idx)

    def queue_size(self, idx):
        return self.queue[idx].size()
    ...
    def start(self, session):
        self._sess = session._tf_sess() if isinstance(session, _MonitoredSession) else session
        super(QueueInputMulti, self).start()
